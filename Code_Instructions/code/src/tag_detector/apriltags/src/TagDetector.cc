...
namespace AprilTags {
  //Initial function added
  void TagDetector::init(int width, int height)
  {
      edges.reserve(width*height*4);
      storage.resize(width*height*4);

      tmin = &storage[width*height*0];
      tmax = &storage[width*height*1];
      mmin = &storage[width*height*2];
      mmax = &storage[width*height*3];
  }
